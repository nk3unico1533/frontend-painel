...
export default function HistoricoFlutuante({ items = [], visible = true }) {
  ...
  useEffect(()=>{
    ...
    const down = (e)=>{
      ...
      startR = pos.right; startB = pos.bottom;
  ...
  return (
    <div ref={ref} className="float-block" style={{ right: pos.right, bottom: pos.bottom }}>
      ...
      <div>
        {items.slice(0,4).map((it, idx)=>(
          <div key={idx} style={{ padding:10, borderRadius:8, marginBottom:8, background:"rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize:13 }}><strong>{it.tipo}</strong> — {it.valor}</div>
            <div style={{ fontSize:12, color:"var(--muted)" }}>{it.criadoEm ? new Date(it.criadoEm).toLocaleString() : "-"}</div>
          </div>
        ))}
